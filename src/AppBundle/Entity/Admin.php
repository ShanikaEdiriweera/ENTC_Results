 ...
 //-----------------------------------------------------------   
    ...
    public function save()
    {
        if ($this->id == null) {
            ...
            $stmt = $con->prepare('INSERT INTO admin (name, username,password, email,is_active) VALUES (?,?,?,?,?)');
            $stmt->bind_param("sssss",$this->name, $this->username, $this-> password, $this->email, $this->isActive);
            $stmt->execute();  
            $stmt->close();
        }else{
            $con = Connection::getConnectionObject()->getConnection();
            $stmt = $con->prepare('UPDATE admin SET (name, username,password, email,is_active) VALUES (?,?,?,?,?)');
            $stmt->bind_param("sssss",$this->name, $this->username, $this-> password, $this->email, $this->isActive);
            $stmt->execute();
            $stmt->close();
        }
    ...
    public static function getOne($id){
        ...
        $stmt = $con->prepare('SELECT name,username,password,email,is_active FROM admin WHERE id=?');
        $stmt->bind_param("s",$id);
        $stmt->execute();

        $stmt->bind_result($ad->name, $ad->username, $ad-> password, $ad->email, $ad->isActive);
        $stmt->fetch();
        $stmt->close();
        return $ad;
    }
